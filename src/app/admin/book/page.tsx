...
const Page = (props: Props) => {
    ...
    useEffect(() => {
        currentUser.position && getItems(currentUser.position, "book", "", undefined, undefined)
    ...
    return (
        <div className='bglv1 pd-5p br-5px'>
            <h3 className='mg-bot-20px'>Blogs
                <span className='fos-70p  hover-link mg-0px-10px cs-p' onClick={() => toPage.push("/admin/book/new")} >make a new book</span>
            </h3>
            {items.map((item, index) =>
                <div key={index} className=' pd-10px bor-bot-1px borC-main mg-10px-0px cs-p'
                    onClick={() => toPage.push("/admin/book/" + item.slug)}>
                    <h4>{item.name}</h4>
